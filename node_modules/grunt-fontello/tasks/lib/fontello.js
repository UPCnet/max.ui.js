...
var createSession = function(options, callback){
  ...
  fs.exists(options.session, function(exists) {
    if (!exists) {
      ...
      fs.readFile(options.session, 'utf8', function(err, data) {
        if (err) throw err;
...
var fetchStream = function(options, session, callback){
  ...
  fs.writeFile(options.session, session, function(err) {
      if(err) {
          console.log(err);
      } else {
          grunt.log.write("Session saved\n");
      }
  }); 

  grunt.log.write('Modify your font at '+ options.host + '/' + session +'\n');
      ...
      .on('entry', function(entry){
        ...
        if(entry.type === 'File'){
          switch(ext){
            ...
            case '.css':
              // SCSS:
              var cssPath = (options.less) ? 
                path.join(options.styles, path.basename(entry.path)) :
                path.join(options.styles, '_' + path.basename(entry.path).replace(ext, '.less'));
